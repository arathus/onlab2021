...
class Backtest:  	
	def __init__(self, strategy, data, from_date, to_date, balance=10000, leverage=0, max_units=500000000, verbose=True, ipynb=False, direct=True, test=False, ddw=0, 
						commission=0.0, rfr=0.02, benchmarking=True):
		# initial variables
		...
		self.BM = benchmarking											# using benchmark in the simulation
		...
		columns=['Type', 'Instrument', 'Open Time', 'Close Time', 'Units', 'Margin Used', 'Open Price', 'Close Price', 'Spread', 'Profit',  'Balance', 'AutoClose', 'TP', 'SL']
	...
	def add_ma(self, n):
		...
		self.Data[name] = self.Data['AC'].rolling(n).mean()


	def add_indicator(self, tp, length):
		if 'DEMA' in tp:
			self.add_dema(length)
		if 'TEMA' in tp:
			self.add_tema(length)
		if 'EMA' in tp:
			self.add_ema(length)
		if 'WMA' in tp:
			self.add_wma(length)
	...
	def buy(self, row, instrument, trade_ammount, stop_loss=0, take_profit=0, units=0):
		...
		self.OpenPositions.append(Trade(instrument, 'BUY', units, row, stop_loss, take_profit, self.Direct))
		return True
	...
	def run(self):
		...
		if len(self.TradeLog) > 0:
			if self.DDW != 0:
				...
				self.TradeLog['Drawdown'] = self.TradeLog['Balance'].rolling(int(dd_length)).apply(self.max_dd)

			columns = ['Nr. of Trades', 'Profit / Loss', 'Profit Factor', 'Win Ratio', 'DDW', 'DDW (%)', 'Buy & Hold', 'Sharpe Ratio', 'Balance', 'Max. Balance', 'Min. Balance', 'Balance Std.', 'Gross Profit', 'Gross Loss', 'Winning Trades', 'Losing Trades', 'Average P/L', 'P/L Std.', 'Average Profit', 'Average Loss', 'Profit Std.', 'Loss Std.', 'SL/TP Activated', 'RoR (10%)', 'RoR (20%)', 'RoR (30%)', 'Outlier Sensitivity']
			...
			buy_values = [len(buy), buy['Profit'].sum(), buy[buy['Profit'] > 0]['Profit'].sum() / abs(buy[buy['Profit'] < 0]['Profit'].sum()), 
					len(buy[buy['Profit'] > 0]) / self.length(buy), None, None, None, None, None, None, None, None, 
			...
			sell_values = [len(sell), sell['Profit'].sum(), sell[sell['Profit'] > 0]['Profit'].sum() / abs(sell[sell['Profit'] < 0]['Profit'].sum()), 
					len(sell[sell['Profit'] > 0]) / self.length(sell), None, None, None, None, None, None, None, None,
			...
			BnH = (self.Data['BC'][len(self.Data)-1] - self.Data['AC'][0]) * (1 / self.Data['BC'][len(self.Data)-1]) * self.InitBalance * self.Leverage
			if not self.Direct:
				BnH = (self.Data['BC'][len(self.Data)-1] - self.Data['AC'][0]) * self.InitBalance * self.Leverage / self.Data['AC'][0]

			sharpe_ratio = (self.Balance / self.InitBalance - 1 - self.RfR) / (self.TradeLog['Balance'] / self.InitBalance).std()

			wr = len(self.TradeLog[self.TradeLog['Profit'] > 0]) / len(self.TradeLog)
			lr = 1 - wr 
			avg_loss = abs(self.TradeLog.loc[self.TradeLog['Profit'] < 0]['Profit'].mean())
			avg_win = abs(self.TradeLog.loc[self.TradeLog['Profit'] > 0]['Profit'].mean())
			c = self.InitBalance
			...
			profit = self.TradeLog.sort_values(by='Profit', ascending=False)
			...
			all_values = [len(self.TradeLog), self.TotalPL, self.GrossProfit / abs(self.GrossLoss),  wr, 
					self.TradeLog['Drawdown'].min(), abs(self.TradeLog['Drawdown'].min()) / self.TradeLog['Balance'].max(),
					BnH, sharpe_ratio, self.Balance, self.TradeLog['Balance'].max(), self.TradeLog['Balance'].min(), self.TradeLog['Balance'].std(), self.GrossProfit, self.GrossLoss, 
					len(self.TradeLog[self.TradeLog['Profit'] > 0]), len(self.TradeLog[self.TradeLog['Profit'] < 0]), self.TradeLog['Profit'].mean(), self.TradeLog['Profit'].std(),
					...
					self.AutoCloseCount, RoR10, RoR20, RoR30, OLS]
	...
	def plot_results(self, name='backtest_result.html'):    	
		if (len(self.TradeLog) > 0):
			fig = subplots.make_subplots(rows=3, cols=3, shared_xaxes=True,
			...
			price_plot_low = go.Scatter(x=self.Data['Date'] + ' ' + self.Data['Time'], y=self.Data['AL'], name='Price', 
									connectgaps=True, marker=dict(color='#b7c0fa'))
			price_plot_high = go.Scatter(x=self.Data['Date'] + ' ' + self.Data['Time'], y=self.Data['AH'], name='Price', 
									connectgaps=True, marker=dict(color='#b7c0fa'))
			...
			not_needed = ['TP', 'SL', 'AutoClose', 'Drawdown', 'Instrument']
			...
			benchmark_plot = go.Scatter(x=pd.concat([pd.Series(self.Data['Date'][0] + ' ' + self.Data['Time'][0]), self.TradeLog['Close Time']]), 
										y=pd.concat([pd.Series([self.InitBalance]), self.TradeLog['Balance']]) / self.InitBalance - 1, name='BM', 
										connectgaps=True, line_color="#5876F7")
			
			
			# calculating monte carlo simulation
			balance_reference_plot = go.Scatter(x=[x for x in range(len(self.TradeLog))], y=self.TradeLog['Balance'], name='MC Ref', line_color="#5876F7", mode='lines')
			...
			fig.append_trace(price_plot_low, 3, 1)
			fig.append_trace(price_plot_high, 3, 1)
	...
	def plot_indicators(self, name='backtest_indicators.html'):
		if len(self.IndicatorList) > 0:
			...
			fig.update_layout(xaxis_rangeslider_visible=False, title=go.layout.Title(text = self.FromDate[:4] + ' - ' + self.ToDate[:4] + ' Indicators', xref="paper"))
			if self.ipynb:
				iplot(fig)
			else:
				plot(fig, filename=name)
